...
func (u *UploadFileController) Post() {
	...
	fmt.Println("是不是空的",phone)
	//1、解析用户上传的数据及文件内容
	//用户上传的自定义标题
	title := u.Ctx.Request.PostFormValue("upload_title")//用户上传的自定义的标题
	fmt.Println("电子数据标签：", title)
	//用户上传的文件
	file, header, err := u.GetFile("Ander")

	if err != nil { //解析客户端提交的文件发送错误
	...
	if err != nil {
		fmt.Println(err.Error())
		u.Ctx.WriteString("抱歉，文件数据 认证失败，请重试")
		return
	}
	...
	if err != nil {
		...
		u.Ctx.WriteString("抱歉，电子用户认证失败，请重新测试")
		return
	}
	...
	if err != nil {
		...
		u.Ctx.WriteString("抱歉，电子数据保存失败， 请稍后重试")
		return
	}
...
func (u *UploadFileController) Post1() {
	...
	//fromFile: 文件，
	//toFile: 要保存的文件路径
	err = u.SaveToFile("Ander", saveName)
	if err != nil {
		fmt.Println(err.Error())
